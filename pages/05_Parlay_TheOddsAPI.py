# Parlay Builder — TheOddsAPI (merged, improved display with teams/players and bookmaker)
...
def auto_pick_safest_legs(
    ...
    for ev in events:
        ...
        for o in get_consensus_and_best_outcomes(ev):
            ...
            if avoid_same_event and selection in seen_selection_for_event:
                continue
            seen_selection_for_event.add(selection)
...
with left_col:
    ...
    if st.button("Preview safest picks"):
        pool = auto_pick_safest_legs(st.session_state.events, n_legs=int(n_legs), max_decimal=float(max_decimal), min_imp=float(min_imp_pct), max_spread=float(max_spread_pct), require_bm=(require_bm if require_bm.strip() else None), avoid_same_event=avoid_same)
        if not pool:
            ...
            for i, p in enumerate(pool[:50], 1):
                ...
                bm_text = f" — {p.get('best_bookmaker')} @ {p.get('best_book_price'):.2f}" if (show_bookmaker and p.get("best_bookmaker") and p.get("best_book_price")) else ""
    ...
    if st.button("Show top safe candidates"):
        candidates = auto_pick_safest_legs(st.session_state.events, n_legs=200, max_decimal=5.0, min_imp=0.0, max_spread=1.0, require_bm=(require_bm if require_bm.strip() else None), avoid_same_event=False)
        ...
        for i, c in enumerate(candidates[:top_n], 1):
            ...
            bm_text = f" — {c.get('best_bookmaker')} @ {float(c.get('best_book_price')):.2f}" if (show_bookmaker and c.get("best_bookmaker") and c.get("best_book_price")) else ""
